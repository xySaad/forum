...
export const CommentContainer = (comment, postId) => {
  const reactionEndpoint = `/api/reactions/posts/${postId}/comments/`;
  const [like, onLike] = reaction("like", comment, reactionEndpoint);
  const [dislike, onDislike] = reaction("dislike", comment, reactionEndpoint);
...
const fetchComments = async (commentsList, postId) => {
  ...
  json.forEach((comment) => {
    commentsList.add(CommentContainer(comment, postId));
  });
};
